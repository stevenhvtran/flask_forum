...
def test_username_length_errors(client, username):
    response = client.post('/api/register', json={'username': username, 'password': 'test123'})
...
def test_username_already_exists_errors(client_with_user):
    response = client_with_user.post('/api/register', json={'username': 'test123', 'password': 'test123'})
    assert response.status_code == 400
    assert dict(response.get_json()) == {'error': 'Invalid username or password'}
